...
class TableLinesRemover:
    ...
    def execute(self):
        ...
        self.remove_noise_with_morphological_operations()
    ...
    def erode_vertical_lines(self):
        hor = np.ones((1, 15), dtype=np.uint8)
        self.vertical_lines_eroded_image = cv2.erode(self.inverted_image, hor, iterations=3)
        self.vertical_lines_eroded_image = cv2.dilate(self.vertical_lines_eroded_image, hor, iterations=3)

    def erode_horizontal_lines(self):
        ver = np.ones((15, 1), dtype=np.uint8)
        self.horizontal_lines_eroded_image = cv2.erode(self.inverted_image, ver, iterations=3)
    ...
    def remove_noise_with_morphological_operations(self):
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))
        self.image_without_lines_noise_removed = cv2.morphologyEx(self.image_without_lines, cv2.MORPH_OPEN, kernel)
        self.image_without_lines_noise_removed = cv2.morphologyEx(self.image_without_lines_noise_removed, cv2.MORPH_CLOSE, kernel)
